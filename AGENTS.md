# AGENTS.md — Rider Calc · PedidosYa GT

This document provides an overview of the project structure for developers and AI agents working on this codebase.

## Project Overview

Rider Calc is a delivery earnings calculator and tracker for PedidosYa riders in Guatemala. Built with TanStack Start (SSR React) + vanilla JS app logic, deployed as a PWA on Netlify.

The core app logic lives in `public/rider-app.js` — plain vanilla JS that manipulates the DOM after React mounts. React provides the structural JSX scaffold, PWA shell, and Netlify SSR; vanilla JS drives all interactivity and localStorage persistence.

## Architecture Decision: Hybrid React + Vanilla JS

All interactive behavior (calculations, history, stats, charts) is in `public/rider-app.js` which exposes `window.initRiderApp()` and all handler functions. The React component (`src/routes/index.tsx`) calls `window.initRiderApp()` in a `useEffect` after mount, and all `onClick`/`onInput` JSX handlers delegate to `(window as any).fn()`.

This avoids a full React rewrite while gaining the PWA/Netlify infrastructure.

## Data Persistence
All data is in `localStorage`:
- `riderAllOrders` — all order objects
- `riderConfig` — meta, fuel cost config
- `riderRestaurants` — saved restaurant names (merged with built-in GT chains)
- `riderSchedules` — auto-multiplier schedule
- `riderRecord` — personal best earnings day
- `riderOdo_<dateString>` — odometer reading per day

Backup/restore via `window.exportBackup()` / `window.importBackup()` dumps/loads all of the above as JSON.

## PWA / APK Distribution Path
1. Deploy to Netlify (CI auto-builds)
2. `public/manifest.json` + `public/sw.js` enable installability and offline use
3. Chrome on Android shows "Add to Home Screen" for instant PWA install
4. For a signed APK: visit pwabuilder.com → enter the Netlify URL → package for Android
